...
node('linux') {
    stage("Test Stack") {
       ...
       sshagent(['8d1f2576-2d78-4aa7-9782-8e8911d38127']) {
        // some block test
        sh 'ssh -o StrictHostKeyChecking=no ubuntu@manager1PublicIP uptime'           
       }
    }
    stage("Deploy Redis") {
       sh 'docker ps -a'
    ...
    stage("Test Redis") {
       sshagent(['8d1f2576-2d78-4aa7-9782-8e8911d38127']) {
        // some block     
       //sh 'ssh ubuntu@34.233.121.37:6379 exec redis-cli set hello world'
       sh 'ssh ubuntu@34.233.121.37 exec redis-cli -h 34.233.121.37 -p 6379 set hello world'
