...
node('linux') {
    ...
    stage("Test Stack") {
       ...
       sshagent(['8d1f2576-2d78-4aa7-9782-8e8911d38127']) {
        // Check for uptime
        sh 'ssh -o StrictHostKeyChecking=no ubuntu@34.224.70.117 uptime'
       }
    }
    stage("Deploy Redis") {
       sshagent(['8d1f2576-2d78-4aa7-9782-8e8911d38127']) {
           sh 'docker ps -a'
           //sh 'ssh ubuntu@34.224.70.117 \' docker stop $(docker ps -a -q --filter ancestor=redis)\''     
           //sh 'ssh ubuntu@34.224.70.117 \' docker rm $(docker ps -a -q --filter ancestor=redis)\''             
           sh 'ssh ubuntu@34.224.70.117 \' docker run --name redisimage -d redis:latest -p 6379:6379 \''
           }
       }
    stage("Test Redis") {
       sshagent(['8d1f2576-2d78-4aa7-9782-8e8911d38127']) {
       ...
       sh 'ssh ubuntu@34.224.70.117 \' exec redis-cli set hello world\''
