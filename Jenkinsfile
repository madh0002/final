...
//sh 'aws cloudformation wait stack-create-complete --stack-name final-test --region us-east-1'
       ...
       sshagent(['8d1f2576-2d78-4aa7-9782-8e8911d38127']) {
        // some block test
        sh 'ssh -o StrictHostKeyChecking=no ubuntu@54.146.88.90 uptime'           
       }
    }
    stage("Deploy Redis") {
       sh 'docker ps -a'
       ...
       sh 'docker run --name redisimage -d redis:latest -h 54.146.88.90 -p 6379:6379'
       sh 'docker images'
    ...
    stage("Test Redis") {
       sh 'docker ps -a'        
       sshagent(['8d1f2576-2d78-4aa7-9782-8e8911d38127']) {
       //sh 'ssh ubuntu@54.146.88.90 \' redis-cli -h 54.146.88.90 set hello world\''
