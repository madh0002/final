...
sh 'aws cloudformation wait stack-create-complete --stack-name final-test --region us-east-1'
       ...
       sshagent(['8d1f2576-2d78-4aa7-9782-8e8911d38127']) {
        // some block test
        sh 'ssh -o StrictHostKeyChecking=no ubuntu@3.85.98.74 uptime'           
       }
    }
    stage("Deploy Redis") {
       sh 'docker ps -a -q'
       ...
       sh 'docker run --name redisimage -d redis:latest -h 3.85.98.74 -p 6379:6379'
    }
    stage("Test Redis") {
       sh 'docker ps -a -q'        
       //sshagent(['8d1f2576-2d78-4aa7-9782-8e8911d38127']) {
       sh 'ssh ubuntu@3.85.98.74 \' redis-cli set hello world\''
